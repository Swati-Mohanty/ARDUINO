...
//with  serial input
//displays current
//average resistance display when 0.49 < vout1 < 0.51
    ...
    if(check_bit==1)
    {
      ...
      for(int b=0;b<3;b++)
      {
        ...
        if(b==0)
        {
          for(int a=0;a<39;a++)
          {
            if((vout1_ramp[b][a]>0.48)&&(vout1_ramp[b][a]<0.51))
            {
              float curr = ((vout2_ramp[b][a]-vout1_ramp[b][a])/(Rref1*1E3)) + c_t*((vout2_ramp[b][a]-vout2_ramp[b][a+1])-(vout1_ramp[b][a]-vout1_ramp[b][a+1]));
              curr = curr*1E6;
        ...
        else if(b==1)
        {
          for(int a=0;a<39;a++)
          {
            if((vout1_ramp[b][a]>0.48)&&(vout1_ramp[b][a]<0.51))
            {
              float curr = ((vout2_ramp[b][a]-vout1_ramp[b][a])/(Rref2*1E3)) + c_t*((vout2_ramp[b][a]-vout2_ramp[b][a+1])-(vout1_ramp[b][a]-vout1_ramp[b][a+1]));
              curr = curr*1E6;
              Serial.print(vout1_ramp[b][a]);
              Serial.print(" ");
              Serial.print(vout2_ramp[b][a]); 
        ...
        else if(b==2)
        {
          for(int a=0;a<39;a++)
          {
           if((vout1_ramp[b][a]>0.48)&&(vout1_ramp[b][a]<0.51))
            {
              float curr = ((vout2_ramp[b][a]-vout1_ramp[b][a])/(Rref3*1E3)) + c_t*((vout2_ramp[b][a]-vout2_ramp[b][a+1])-(vout1_ramp[b][a]-vout1_ramp[b][a+1]));
              curr = curr*1E6;
              Serial.print(vout1_ramp[b][a]);
              Serial.print(" ");
              Serial.print(vout2_ramp[b][a]); 
